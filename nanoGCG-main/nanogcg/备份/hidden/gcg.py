...
class GCG:
    def __init__(
        ...
        # 强制 forward 返回 hidden states，供 SOM-IRIS loss 使用。
        ...
        logger.info("[✓] Model forward patched to always output hidden_states.")
        # ======================================================
    ...
    def run(
        ...
        if config.use_prefix_cache:
            with torch.no_grad():
                output = model(inputs_embeds=before_embeds, use_cache=True,output_hidden_states=True)
            ...
            with torch.no_grad():
                ...
                if self.config.probe_sampling_config is None:
                    ...
                    print(
                        ...
                        f"🎯 Target CE Loss: {self.last_target_losses[best_idx].item():.4f} | "
                        f"🛡️ SOM Loss: {self.last_som_losses[best_idx].item():.6f} | "
                        f"Weighted SOM: {((self.beta if self.use_beta_loss else self.som_coef) * self.last_som_losses[best_idx]).item():.4f}"
                    )
                else:
    ...
    def compute_som_iris_loss(
        ...
        # In both cases, prompt_end is the index right after the full input prompt
        # and right before target tokens.
        prompt_end = input_len - target_len

        token_positions = []

        # IRIS-like: last input token immediately before target tokens.
        if self.som_token_mode in ["last_input", "both"]:
            token_positions.append(prompt_end - 1)

        # Whole suffix interval. This is useful if last_input is mostly template text.
        ...
        token_positions = sorted(set(p for p in token_positions if 0 <= p < input_len))
    ...
    def compute_token_gradient(
        ...
        """Compute gradient of target CE + SOM-IRIS penalty w.r.t. one-hot suffix tokens."""
        model = self.model
        embedding_layer = self.embedding_layer
        ...
        if self.config.use_mellowmax:
            ...
            target_loss = mellowmax(-label_logits, alpha=self.config.mellowmax_alpha, dim=-1)
        else:
        ...
        # 2. IRIS-style SOM refusal projection penalty.
        ...
        # 3. Joint objective. Do not use beta here; use explicit scale control.
    ...
    def _compute_candidates_loss_original(
        ...
        """Compute target CE + SOM-IRIS loss for all candidate suffixes."""
        all_loss = []
        all_target_loss = []
        all_som_loss = []
        prefix_cache_batch = []
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                ...
                if self.prefix_cache:
                    ...
                    outputs = self.model(inputs_embeds=input_embeds_batch, output_hidden_states=True)
