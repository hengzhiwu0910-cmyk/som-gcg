...
class GCGConfig:
   ...
   # ===== SOM / IRIS config =====
    beta: float = 0.05
    use_beta_loss: bool = True
    som_coef: float = 0.0

    # "last_input" is closest to IRIS:
    # penalize the last prompt token right before target tokens.
    som_token_mode: str = "last_input"
    

    # None means using default all layers: list(range(1, 33))
    iris_layers: Optional[List[int]] = None

    # SOM direction indices selected from SOM directions.
    best_k_indices: Optional[List[int]] = None

    # Use IRIS-style residual activation sites.
    # True is closer to IRIS; False is hidden_states approximation.
    use_resid_write_hooks: bool = False
    # Scale SOM loss after reduction.
...
class GCG:
    def __init__(
        ...
        self.iris_layers = (
            list(range(1, 33))
            if getattr(config, "iris_layers", None) is None
            else list(config.iris_layers)
        )

        self.use_beta_loss = bool(getattr(config, "use_beta_loss", True))
        self.beta = float(getattr(config, "beta", 0.25))
        self.som_coef = float(getattr(config, "som_coef", 0.0))

        self.som_token_mode = str(getattr(config, "som_token_mode", "last_input"))
        ...
        self.best_k_indices = (
            [10, 6, 8, 13, 14, 15, 9]
            if getattr(config, "best_k_indices", None) is None
            else list(config.best_k_indices)
        )
        ...
        logger.info(
            ...
            f"Shape: {self.som_directions.shape}; "
            f"use_beta_loss={self.use_beta_loss}; beta={self.beta}; coef={self.som_coef}; "
            f"token_mode={self.som_token_mode}; layers={self.iris_layers}; "
            f"use_resid_write_hooks={getattr(config, 'use_resid_write_hooks', True)}; "
            f"som_loss_scale={getattr(config, 'som_loss_scale', 1.0)}"
        )
    ...
    def compute_som_iris_loss(
        ...
        for layer_idx in self.iris_layers:
            ...
            h_layer = hidden_states_pool[layer_idx]  # 不要先整体 float

            for pos in token_positions:
                h = h_layer[:, pos, :].float()       # 只转需要的位置
                proj = h @ dirs.T
    ...
    def compute_token_gradient(
        ...
        need_hidden_states = self.enable_som_loss and (not self.use_resid_write_hooks)
