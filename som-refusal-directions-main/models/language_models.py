...
class LanguageModel(ABC):
    ...
    def generate_completions(self, dataset, fwd_pre_hooks=None, fwd_hooks=None, max_new_tokens=64, batch_size=16):
        """
        支持动态挂载 Hook 的生成函数，用于实时干预模型推理。
        """
        completions = []
        ...
        for module, hook in fwd_pre_hooks:
            pre_handles.append(module.register_forward_pre_hook(hook))

        try:
            for i in tqdm(range(0, len(dataset), batch_size)):
                prompts = [self._get_prompt(prompt=inst) for inst in instructions[i:i + batch_size]]

                # 显式处理 padding 和 device
                tokenized = self.tokenizer(prompts, padding=True, return_tensors="pt").to(self.device)

                with torch.no_grad():
                    gen_toks = self.model.generate(
                ...
                for idx, gen in enumerate(gen_toks):
                    completions.append({
